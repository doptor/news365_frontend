                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        {item.key === openSubMenu ?
                                            <div
                                                className="absolute left-0 top-full pt-2 w-48 bg-white border rounded shadow-md z-50"
                                                onMouseLeave={() => setOpenSubMenu(null)}> {/* Mega menu */}
                                                <ul className="list-none p-2">
                                                    {item.subMenu.map((subItem: { label: string, href: string }) => (
                                                        <li key={subItem.label}>
                                                            <Link href={subItem.href}
                                                                  className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">{subItem.label}</Link>
                                                        </li>
                                                    ))}
                                                </ul>
                                            </div>
